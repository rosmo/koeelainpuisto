...
for file_name in sys.argv[2:len(sys.argv)]:
    print('Processing: %s (%d/%d)' % (file_name, file_index, total_files))
    file_index += 1
    ...
    if process_speech:
        for segment in file_info['segments']['speech']:
            ...
            if not caption_key in file_info['captions']:
                if (segment[1] - segment[0]) > 300:
                    print('Skipping segment, too long (%s)' % caption_key)
                    continue
                segment_start = int(samples_per_second * (segment[0] - 1))
                if segment_start < 0:
                    segment_start = 0
                ...
                try:
                    ...
                    file_info['captions'][caption_key] = ''
    ...
    if process_music:
        for segment in file_info['segments']['music']:
            ...
            if not caption_key in file_info['captions']:
                ...
                print(result.text)
                if 'result' in results_json and results_json['result'] is not None and 'artist' in results_json['result']:
                    song_name = ''
                    ...
                    file_info['captions'][caption_key] = ''
                    file_info['songs'][caption_key] = ''

                json_output = 'json/%s.json' % (file_name_ya)  
                with open(json_output, 'w') as outfile:
                    print('Writing to %s' % (json_output))
                    json.dump(file_info, outfile, indent=4, sort_keys=True)
    print('Processing complete: %s' % (file_name_ya))
